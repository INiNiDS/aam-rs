...
unsafe fn get_aam_mut<'a>(ptr: jlong) -> Option<&'a mut AAM> {
    if ptr == 0 {
        ...
        // SAFETY: the pointer is only ever produced by `Box::into_raw` and is
        // checked against null by callers that have ensured it is still valid
        // and that no aliasing reference is held concurrently.
...
fn aam_reload_impl(mut env: Env, ptr: jlong, content: JString) {
    ...
    if let Err(e) = aam.update_from_text(&content_str) {
        let msg = e
            .into_iter()
            .next()
            .map(|err| err.to_string())
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_rs_in_ininids_aam_1jv_AAM_update(env: Env, _class: JClass, ptr: jlong) {
    aam_update_impl(env, ptr)
}

#[unsafe(no_mangle)]
pub extern "system" fn Java_rs_in_ininids_aam_1jv_AAM_reload(
    env: Env,
    _class: JClass,
    ptr: jlong,
    content: JString,
) {
    aam_reload_impl(env, ptr, content)
}
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_rustgames_aam_AAM_update(env: Env, _class: JClass, ptr: jlong) {
    aam_update_impl(env, ptr)
}

#[unsafe(no_mangle)]
pub extern "system" fn Java_com_rustgames_aam_AAM_reload(
    env: Env,
    _class: JClass,
    ptr: jlong,
    content: JString,
