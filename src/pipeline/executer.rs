    ...
    fn apply_schema(
        ...
        for key in root_keys {
            for (field, (type_name, is_optional)) in &schema.fields {
                ...
                if let Some(value) = output_map.get(full_key.as_str()) {
                    if let Err(err_msg) =
                ...
                for (field, (type_name, _is_optional)) in &schema.fields {
                    ...
                    if !output_map.contains_key(full_child_key.as_str()) {
                        if let Some(default_val) =
                            context.default_value_for_type(type_name.as_str())
                        {
                            output_map
                                .insert(SmolStr::new(&full_child_key), SmolStr::new(&default_val));
                        }
                    }
                }
            }
